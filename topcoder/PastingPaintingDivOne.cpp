...
class PastingPaintingDivOne {
	...
	vector<long long> countColors(vector <string> clipboard, int T) {
		n = clipboard.size(); m = clipboard[0].size(); int L = max(n, m); L = min(L, T);
		for (int i = 0; i < n; i++)
			for (int j = 0; j < m; j++) a[i + 1][j + 1] = Cal(clipboard[i][j]);
		int d[4] = {0}, s[4] = {0};
		for (int k = 1; k <= L; k++) {
			memset(d, 0, sizeof d);
			for (int i = k; i <= k + n - 1; i++)
				for (int j = k; j <= k + m - 1; j++) if (a[i - k + 1][j - k + 1]) {
					s[a[i - k + 1][j - k + 1]]++; s[b[i][j]]--;
					d[a[i - k + 1][j - k + 1]]++; d[b[i][j]]--;
					b[i][j] = a[i - k + 1][j - k + 1];
				}
		}
		...
		for (int i = 1; i <= 3; i++) {
			long long tmp = s[i];
			tmp += (long long)(T - L) * (long long)d[i];
			ret.push_back(tmp);
		}
		return ret;
	}
...
}; vector <string> Arg0(Arr0, Arr0 + (sizeof(Arr0) / sizeof(Arr0[0]))); int Arg1 = 1000000000; long long Arr2[] = {2000000018, 17000000048ll, 2000000005 }; vector<long long> Arg2(Arr2, Arr2 + (sizeof(Arr2) / sizeof(Arr2[0]))); verify_case(5, Arg2, countColors(Arg0, Arg1)); }
