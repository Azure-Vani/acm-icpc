...
namespace Solve {
	...
	inline void Input(void) {
		...
		scanf("%d\n", &n); scanf("%s\n", ch); std::reverse(ch, ch + n);
		REP(i, 0, n - 1) l = i / 4, a[l] += (ch[i] - 48) * pow[i % 4];scanf("%s\n", ch); std::reverse(ch, ch + n);
	...
	inline void Init(void) {
		e[0] = ii(1, 0);
		for(int i = 1; i < n; i++) e[i] = ii(cos(pi * i * 2 / n), sin(pi * i * 2 / n));
		REP(i, 1, n - 1) e[-i] = e[n - i];
	}

	inline void Dft(ii *a, ii *w, int L, int d) {
		register ii t;
